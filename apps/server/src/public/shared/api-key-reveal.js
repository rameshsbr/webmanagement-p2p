(function () {
  ...
  class KeyRevealController {
    constructor(modal) {
      this.modal = modal;
      this.scopeKey = (modal.getAttribute('data-scope') || '').trim();
      this.scope = this.scopeKey || DEFAULT_SCOPE;
  ...
  const modals = Array.from(document.querySelectorAll('[data-api-key-reveal-modal]'));
  if (!modals.length) return;

  const controllerEntries = modals
    .map((modal) => {
      const controller = getController(modal);
      if (!controller) return null;
      return { scope: normalizeScope(controller.scopeKey), controller };
    })
    .filter(Boolean);

  if (!controllerEntries.length) return;

  const triggers = Array.from(document.querySelectorAll('[data-api-key-reveal-trigger]'));
  if (!triggers.length) return;

  const fallbackController = controllerEntries[0].controller;

  triggers.forEach((trigger) => {
    const triggerScope = normalizeScope(
      trigger.getAttribute('data-reveal-scope') || trigger.getAttribute('data-scope') || ''
    );
    const entry = triggerScope
      ? controllerEntries.find((item) => item.scope === triggerScope)
      : controllerEntries.find((item) => !item.scope);
    const controller = (entry && entry.controller) || fallbackController;
