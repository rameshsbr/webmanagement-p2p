...
router.get('/export/deposits.csv', async (req: Request, res: Response) => {
  ...
  for (const x of items) {
    ...
    csv.write({
      ...
      processedAt: processedAtDate ? processedAtDate.toISOString() : '',
      processingSeconds: processingSeconds ?? '',
      processedBy: x.processedByAdmin ? (x.processedByAdmin.displayName || x.processedByAdmin.email || x.processedByAdmin.id) : '',
...
router.get('/export/deposits.xlsx', async (req: Request, res: Response) => {
  ...
  items.forEach(x => {
    const processedAt = x.processedAt || x.updatedAt;
    const processedAtDate = processedAt ? new Date(processedAt as any) : null;
    const processingSeconds = processedAtDate ? Math.max(0, Math.round((processedAtDate.getTime() - x.createdAt.getTime()) / 1000)) : '';
    ws.addRow({
      id: x.id,
